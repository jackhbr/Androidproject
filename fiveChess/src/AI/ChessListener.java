...
public class ChessListener extends MouseAdapter implements Config ,ActionListener{
	...
	private JPanel JP;
	int clickFlag=0;//点击有效的标志位，1代表有效
						...
						if(manRobot==true && clickFlag==1)
						  {
							clickFlag=0;  //及时清除这个点击有效的标志位，不然后面就会出错了
								...
								for(int i=0;i<Config.ROWS;i++)
								{
									for(int j=0;j<Config.COLUMNS;j++)
									{
										if(chessValue[j][i]>=maxvalue)  //放入第一个框中的应该是水平的x,而此时的j才是x.
										{
											maxvalue=chessValue[j][i];   //行数和列数输出的不对，不知道是画的问题，还是哪里的问题，差了一个
											maxhang=i;//这里的j，才是坐标里的x ,行就是y,x就是列
											maxlie=j;
										}
